...
class KeyJointDataset(Dataset):
    ...
    def _balanced_sampling_strategy(self):
        ...
        # 按指令分组文件
        instruction_files = {'wave': [], 'welcome': [], 'sayhi': [], 'thumbsup': []}
        for file_name in self.file_names:
            ...
            elif 'sayhi' in file_name:
                instruction_files['sayhi'].append(file_name)
            elif 'thumbsup' in file_name:
                instruction_files['thumbsup'].append(file_name)
...
class KeyJointACTLoss(nn.Module):
    ...
    def __init__(self, 
        ...
        # 指令ID到名称的映射
        self.id_to_instruction = {0: 'wave', 1: 'welcome', 2: 'sayhi', 3: 'thumbsup'}
...
def train_key_joint_act_model(args=None):
    ...
    file_names = [f'wave_{i:03d}.json' for i in range(1, 9)] + [f'welcome_{i:03d}.json' for i in range(1, 9)] + [f'sayhi_{i:03d}.json' for i in range(1, 9)] + [f'thumbsup_{i:03d}.json' for i in range(1, 9)]
